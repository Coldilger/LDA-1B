...
## RoboCasa cross-check: see eval/robocasa/RESULTS.md

Moved 2026-08-18 to its own file (`eval/robocasa/RESULTS.md`) -- it isn't a
Bridge result. Short version: LDA-1B's own published RoboCasa checkpoint
scores 48% on the authors' own benchmark, through this same codebase --
confirming the Bridge 0% above isn't attributable to the codebase,
environment, or hardware. Also there: the camera-viewpoint hypothesis
(RoboCasa is egocentric, Bridge is third-person; LDA's paper documents only
egocentric setups and never mentions Bridge) and the setup notes for
reproducing the RoboCasa run.
